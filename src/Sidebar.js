...
// The brand SVG (kept identical to the existing Dashboard.js / Admin_Menu.php icon).
const BrandMark = ( { size = 22 } ) => (
    <svg width={ size } height={ size } viewBox="0 0 200 200" fill="#6B66F7" aria-hidden="true">
...
const Sidebar = ( { view, onNavigate } ) => {
    ...
    const activeModuleId = isDashboard ? null : view;
    const version = ( typeof cartickAdminSettings !== 'undefined' && cartickAdminSettings.version ) || '';

    return (
        <aside className="cartick-sidebar">
            <div className="cartick-sidebar__sticky">
                <div className="cartick-sidebar__brand">
                    <BrandMark size={ 26 } />
                    <span className="cartick-sidebar__brand-name">Cartick</span>
                </div>

                <nav className="cartick-sidebar__nav">
                <a
                    href={ viewToHref( 'dashboard' ) }
                    className={ `cartick-sidebar__item ${ isDashboard ? 'is-active' : '' }` }
                >
                    <HomeIcon width="16" height="16" />
                    <span className="cartick-sidebar__label">Dashboard</span>
                </a>

                <button
                    type="button"
                    className={ `cartick-sidebar__item cartick-sidebar__group-toggle ${ activeModuleId ? 'is-active' : '' }` }
                    onClick={ () => setModulesOpen( ( o ) => ! o ) }
                    aria-expanded={ modulesOpen }
                >
                    <GridIcon width="16" height="16" />
                    <span className="cartick-sidebar__label">Modules</span>
                    <ChevronDownIcon
                        width="12"
                        height="12"
                        className={ `cartick-sidebar__chev ${ modulesOpen ? '' : 'is-collapsed' }` }
                    />
                </button>

                { modulesOpen && (
                    <div className="cartick-sidebar__sub">
                        { MODULES.map( ( m ) => (
                            <a
                                key={ m.id }
                                href={ viewToHref( m.id ) }
                                className={ `cartick-sidebar__sub-item ${ activeModuleId === m.id ? 'is-active' : '' }` }
                            >
                                { m.name }
                            </a>
                        ) ) }
            ...
            { version && (
                <div className="cartick-sidebar__foot">
                    <span className="cartick-sidebar__version">v{ version }</span>
                </div>
            ) }
        </aside>
    );
