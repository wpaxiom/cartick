<?php
/**
 * Cartick Admin Menu
 *
 * @package cartick
 * @author WpAxiom <info@wpaxiom.com>
 * @version 1.0.0
 * @since 1.0.0
 */

namespace WpAxiom\Cartick\Admin;

if ( ! defined( 'ABSPATH' ) ) exit;

/**
 * Admin Menu Class
 */
class Admin_Menu {

	/**
	 * Known module ids. Mirrors `MODULES[i].id` in src/modules.js — used
	 * only to validate ?module=<id> so a bad value falls back to dashboard.
	 *
	 * Module pages are NOT registered as WP submenus. They share the
	 * single `cartick-options` page and disambiguate via ?module=<id>.
	 * This keeps the WP admin sidebar to a single Cartick item even as
	 * the module catalogue grows.
	 */
	const KNOWN_MODULES = array(
		'add-to-cart',
		'sticky-cart',
		'menu-cart',
		'off-canvas-cart',
	);

	/**
	 * Initialize Admin Menu
	 */
	public function __construct() {
		add_action( 'admin_menu', array( $this, 'admin_menu' ) );
	}

	/**
	 * Admin Menus
	 */
	public function admin_menu() {
		$icon = 'data:image/svg+xml;base64,';

		$hook = add_menu_page(
			'Cartick',
			'Cartick',
			'manage_options',
			'cartick-options',
			array( $this, 'cartick_options_callback' ),
			$icon,
			'5'
		);

		add_action( 'load-' . $hook, array( __CLASS__, 'admin_load' ) );
	}

	/**
	 * Resolve the requested admin page → React view id.
	 * Reads ?module=<id>. Returns the module id if it's known, else 'dashboard'.
	 */
	public static function current_view(): string {
		$module = isset( $_GET['module'] ) ? sanitize_key( wp_unslash( $_GET['module'] ) ) : '';

		if ( '' === $module ) {
			return 'dashboard';
		}
		if ( in_array( $module, self::KNOWN_MODULES, true ) ) {
			return $module;
		}
		return 'dashboard';
	}

	/**
	 * Admin Load
	 */
	public static function admin_load() {
		remove_all_actions( 'admin_notices' );
		add_action( 'admin_enqueue_scripts', array( __CLASS__, 'admin_assets' ) );
	}

	/**
	 * Admin assets
	 */
	public static function admin_assets() {
		wp_enqueue_style( 'cartick-admin-fonts', 'https://fonts.googleapis.com/css2?family=Nunito:wght@400;500;600;700&family=Poppins:wght@500;600&display=swap', array(), CARTICK_VERSION );
		wp_enqueue_style( 'cartick-admin', CARTICK_ASSETS . '/dist/css/admin.css', array( 'cartick-admin-fonts' ), CARTICK_VERSION );
		wp_enqueue_script( 'admin-js', CARTICK_URL . '/build/index.js', array( 'wp-element' ), CARTICK_VERSION, true );
		wp_localize_script( 'admin-js', 'cartickAdminSettings', array(
			'url'     => esc_url_raw( rest_url() ),
			'nonce'   => wp_create_nonce( 'wp_rest' ),
			'menus'   => cartick_nav_menus(),
			'version' => CARTICK_VERSION,
			'view'    => self::current_view(),
			'pageBase' => admin_url( 'admin.php' ),
		) );
	}

	/**
	 * Cartick Callback function
	 */
	public function cartick_options_callback() {

		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You do not have sufficient permissions to access this page.', 'cartick' ) );
		}

		?>
			<div class="wrap">
				<div class="cartick-wrap" id="cartick-wrap">
					<div id="cartick-admin"></div>
				</div><!-- end .cartick-wrap -->
			</div>
		<?php
	}
}

// End of file Admin_Menu.php.
